import Image from "next/image";
import Link from "next/link";
import Balancer from "react-wrap-balancer";

export default function SiteFooter() {
	return (
		<footer className="px-4 py-8 mx-auto max-w-screen-2xl">
			<div className="flex flex-col sm:flex-row items-center sm:items-end text-center gap-y-12 gap-x-4 mb-12">
				<div className="flex-1 z-10 flex flex-col items-center sm:items-stretch text-center sm:text-start">
					<Image
						src="/logo.png"
						height="100"
						width="100"
						alt="piliagency logo"
					/>
					<p className="max-w-xs font-light mt-2">
						<Balancer>
							Vi forbinder brands med influencers og skaber magi.
						</Balancer>
					</p>
				</div>
				<div className="flex-1 z-10">
					<h3 className="text-xl mb-5">Links</h3>
					<ul className="flex flex-wrap gap-3 justify-center">
						<li>
							<Link href="/kontakt">Kontakt</Link>
						</li>
						<li>
							<Link href="/om-os">Om os</Link>
						</li>
						<li>
							<Link href="/cases">Cases</Link>
						</li>
						<li>
							<Link href="/creators">Creators</Link>
						</li>
						<li>
							<Link href="/partners">Partners</Link>
						</li>
					</ul>
				</div>
				<div className="flex-1 z-10 sm:text-end">
					<h3 className="text-xl mb-5">Kontakt</h3>

					<p>Pili Agency</p>
					<p>Rahbeks Alle 21</p>
					<p>1801 Frederiksberg C</p>
					<p>Tlf. 42 19 13 25</p>
					<p>
						Mail: <a href="mailto:ka@piliagency.com">ka@piliagency.com</a>
					</p>
				</div>
			</div>
			<div className="border-t border-gray-300"></div>
			<div className="flex flex-col items-center sm:flex-row text-center sm:text-start pt-8">
				<a
					href="/"
					className="text-xl font-bold leading-none text-gray-900 select-none">
					Pili Agency
				</a>
				<p className="mt-4 text-sm text-gray-600 sm:ml-4 sm:pl-4 sm:border-l sm:border-gray-200 sm:mt-0">
					© {new Date().getFullYear()} Pili Agency - Influencer Marketing.
				</p>
				<span className="inline-flex justify-center mt-4 space-x-5 sm:ml-auto sm:mt-0 sm:justify-start">
					<a
						href="https://www.instagram.com/piliagency/"
						className="text-gray-600 hover:text-gray-500 text-sm flex flex-wrap gap-2"
						target="_blank"
						rel="noopener noreferrer">
						<span>Følg vores instagram</span>
						<svg
							className="w-6 h-6"
							fill="currentColor"
							viewBox="0 0 24 24"
							aria-hidden="true">
							<path
								fillRule="evenodd"
								d="M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.049-1.064.218-1.791.465-2.427a4.902 4.902 0 011.153-1.772A4.902 4.902 0 015.45 2.525c.636-.247 1.363-.416 2.427-.465C8.901 2.013 9.256 2 11.685 2h.63zm-.081 1.802h-.468c-2.456 0-2.784.011-3.807.058-.975.045-1.504.207-1.857.344-.467.182-.8.398-1.15.748-.35.35-.566.683-.748 1.15-.137.353-.3.882-.344 1.857-.047 1.023-.058 1.351-.058 3.807v.468c0 2.456.011 2.784.058 3.807.045.975.207 1.504.344 1.857.182.466.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.054.048 1.37.058 4.041.058h.08c2.597 0 2.917-.01 3.96-.058.976-.045 1.505-.207 1.858-.344.466-.182.8-.398 1.15-.748.35-.35.566-.683.748-1.15.137-.353.3-.882.344-1.857.048-1.055.058-1.37.058-4.041v-.08c0-2.597-.01-2.917-.058-3.96-.045-.976-.207-1.505-.344-1.858a3.097 3.097 0 00-.748-1.15 3.098 3.098 0 00-1.15-.748c-.353-.137-.882-.3-1.857-.344-1.023-.047-1.351-.058-3.807-.058zM12 6.865a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 1.802a3.333 3.333 0 100 6.666 3.333 3.333 0 000-6.666zm5.338-3.205a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4z"
								clipRule="evenodd"></path>
						</svg>
					</a>
				</span>
			</div>
		</footer>
	);
}
